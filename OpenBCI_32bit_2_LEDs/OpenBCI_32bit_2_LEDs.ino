...
int GRN = 11; // green LED attached to pin D11 on OpenBCI board
int RED = 12; // red LED attached to pin D12 on OpenBCI board
boolean REDstate, GRNstate; // used to switch LEDs from on to off to on, etc
unsigned long flashCounter = 0; // used to time frequency of LED flashes
//------------------------------------------------------------------------------
...
void setup(void) {
  ...
  // flashing LEDs at different frequencies. Period needs clean /4mS to work!
  pinMode(GRN,OUTPUT);  
  pinMode(RED,OUTPUT);  
  REDstate = GRNstate = HIGH; // start with the lights on
...
void loop() {
  ...
  if(is_running){
      ...
      if (flashCounter%25 == 0){  // blink the green LED at 5Hz | 200mS period | switch pin state every 25 samples
        GRNstate = !GRNstate;     // toggle state of LED
        digitalWrite(GRN,GRNstate);
      }     
      if (flashCounter%17 == 0){  // blink the red LED at 7.35Hz | 136mS period | switch pin state every 17 samples
